...
def convert_sequence(root, stereo="left"):
    ...
    if not os.path.exists(upimgs):
        cmd = f"python ./rpg_vid2e/upsampling/upsample.py --input_dir={input_dir} --output_dir={upimgs}"
        os.system(f"{cmd}")
...
def main():
    ...
    for i in range(len(ROOTS)):
        ...
        convert_sequence(os.path.abspath(ROOTS[i]), stereo="left")
        print(f"convert_tartan.py: Finished processing {ROOTS[i]}\n\n")


if __name__ == "__main__":
    main()
    message = "training data pre-process finished"
